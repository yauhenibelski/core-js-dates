...
function getNextFriday(date) {
  ...
  if (day >= 4) {
    nextDay = 7 - Math.abs(day - 4) + 1;
  } else {
    nextDay = Math.abs(day - 4) + 1;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return new Date(Date.parse(dateEnd) - Date.parse(dateStart)) / 86400000 + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const min = new Date(year, month - 1, 0);
  let res = 0;

  while (Date.parse(d.toString()) > Date.parse(min.toString())) {
...
function getWeekNumberByDate(date) {
  ...
  const min = new Date(date.getFullYear(), 0, 1);
  let result = 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  let cwd = 0;
  let cfd = countOffDays;
  const res = [];

  while (Date.parse(start.toString()) <= Date.parse(end.toString())) {
    if (cwd < countWorkDays) {
      cwd += 1;
      cfd = countOffDays;
      res.push(
        `${`${start.getDate()}`.padStart(2, '0')}-${`${start.getMonth() + 1}`.padStart(2, '0')}-${start.getFullYear()}`
      );
    }
    if (!cfd) {
      cwd = 0;
